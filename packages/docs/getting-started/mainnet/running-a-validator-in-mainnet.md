...
This section explains how to get a Validator node running on [Mainnet](./).
...
If you do not have the required CELO to lock up, you can try out of the process of creating a validator on the Baklava network by following the [Running a Validator in Baklava guide](../baklava-testnet/running-a-validator-in-baklava.md)
...
At a high level, `ReleaseGold` holds a balance for scheduled release, while allowing the held balance to be used for certain actions such as validating and voting, depending on the configuration of the contract. [Read more about `ReleaseGold`](../../celo-owner-guide/release-gold.md).
...
{% hint style="info" %}
When running the following commands, the Beneficiary keys should be [unlocked](running-a-validator-in-mainnet.md#unlocking).
{% endhint %}
